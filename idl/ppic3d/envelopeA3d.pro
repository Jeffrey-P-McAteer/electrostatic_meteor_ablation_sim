...
for i=0,divAsize(4)-1 do begin
    ...
    ; im(A) now contains J/wp/eps. To calculate ve:
    Asize=size(A)
    ve=imaginary(A)*wp*eps
    for iv=0,Asize(Asize(0))-1 do ve(*,*,*,iv)=ve / $
	( (den0(*,*,*,i)+1)*n0d0 + (den1(*,*,*,i)+1)*n0d1 )
    if Asize(0) gt 1 then $
      A2(*,*,*,i)=total(float(A*conj(A) + ve^2),(size(A))(0)) else $
      A2(*,*,*,i)=float(A*conj(A) + ve^2)
endfor
