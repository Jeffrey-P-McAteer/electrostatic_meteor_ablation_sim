...
IF postScriptDevice THEN BEGIN
if !d.name eq 'PS' then bar=!d.table_size-bar-1  ;reverse color in PS
...
IF KEYWORD_SET(vertical) THEN BEGIN
   ...
   IF KEYWORD_SET(right) THEN BEGIN
      ...
      PLOT, [min,max], [min,max], /NODATA, XTICKS=1, $
         ...
         YRANGE=[min, max], YTITLE=ytitle,_EXTRA=e

      AXIS, YAXIS=1, YRANGE=[min, max], YTICKFORMAT=format, YTICKS=divisions, $
         YTICKLEN=0.1, YSTYLE=1, COLOR=color, CHARSIZE=charsize,_EXTRA=e

   ENDIF ELSE BEGIN

      PLOT, [min,max], alog10([min,max]), /NODATA, XTICKS=1, $
         YTICKS=divisions, XSTYLE=1, YSTYLE=9, $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
         YTICKFORMAT=format, XTICKFORMAT='(A1)', YTICKLEN=0.1 , $
        YRANGE=[min, max], title=title,_EXTRA=e

      AXIS, YAXIS=1, YRANGE=[min, max], YTICKFORMAT='(A1)', YTICKS=divisions, $
         YTICKLEN=0.1, YTITLE=ytitle, YSTYLE=1, COLOR=color, CHARSIZE=charsize,_EXTRA=e
...
ENDIF ELSE BEGIN
   ...
   IF KEYWORD_SET(top) THEN BEGIN
      ...
      PLOT, [min,max], [min,max], /NODATA, XTICKS=divisions, $
         ...
         XRANGE=[min, max], XTITLE=title,_EXTRA=e

      AXIS, XTICKS=divisions, XSTYLE=1, COLOR=color, CHARSIZE=charsize, $
         XTICKFORMAT=format, XTICKLEN=0.1, XRANGE=[min, max], XAXIS=1,_EXTRA=e
   ...
   ENDIF ELSE BEGIN
      ...
      PLOT, [min,max], [min,max], /NODATA, XTICKS=divisions, $
         ...
         XRANGE=[min, max], TITLE=title,_EXTRA=e
