/* 3 Routines below for 
   ...
   in 1-D, 2-D and 3-D.  This routine handles particles with different charges

   The input constant c is usually defined as c = damp_nu / vth * dx

   Also, turn the charge of a particle on if it moves through the vertical 
   boundary.
...
#if NDIM == 1
void vpush_bc1(particle_dist &adv, particle_dist &old, particle_dist &cur, field &Efield, 
	   FTYPE qmrat, FTYPE alpha, particle_misc  &misc, FTYPE damp_rate)
{
  ...
  for (i = 0; i < np; ++i) {
    ...
    FTYPE charge_frac=misc.charge[i];
    if ( charge_frac != 0) {
		...
		FTYPE qmrat, FTYPE alpha, particle_misc &misc, FTYPE damp_rate)
{
  ...
  for (i = 0; i < np; ++i) {
	...
	adv.vx(i) = old.vx(i)+Exi
	  -damp_rate*vxmag*(vx-misc.vx0(i));
	adv.vy(i) = old.vy(i)+Eyi
	  -damp_rate*vxmag*(cur.vy(i)-misc.vy0(i));
	     ...
	     FTYPE qmrat, FTYPE alpha, particle_misc &misc, FTYPE damp_rate)
{
  ...
  for (int i = 0; i < np; ++i) {
    ...
    if ( charge_frac != 0) {
      ...
      // Advance the particle velocities: 
      PTYPE cur_x_loc=cur.x(i);
      PTYPE vxmag;
      if ( cur_x_loc < damp_nx ) {
	adv.vx(i) = old.vx(i) + Exi;
      } else {
	/* Add damping - we're in the damped region: */
	PTYPE vx=cur.vx(i);
	...
	/* Add damping - we're in the damped region: */
	adv.vz(i) = old.vz(i)+Ezi
	-damp_rate*vxmag*(cur.vz(i)-misc.vz0(i));
