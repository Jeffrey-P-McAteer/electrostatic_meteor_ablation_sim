...
#include "eppic-math.h" // for transpose2d
/* 
					  ...
					  MPI_Comm comm) 
{
  ...
  if (mpi_np==1) {
    batch_tridiag_cyclic_solver(a,b,c,rhs,lhs,alpha,beta);
    return;
  }
  ...
  */
  ...
  // following is not transposed, so setup with orig ordering:
  ...
  batch_tridiag_cyclic_solver(a_comm_rows,b_comm_rows,c_comm_rows,
