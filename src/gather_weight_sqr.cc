...
void gather_weight_sqr(ArrayNd<OTYPE, n_dist1> &den, particle_dist &pic, 
		   PTYPEAVec &weight, FTYPE *xmin, FTYPE *xmax, 
		   int *nmesh, int *iwrap, FTYPE scale)
{
  ...
  for (int i = 0; i < np; ++i) {
	...
	{
	  w=wx[0][i0];
	  den(ix[0][i0]) += w*Sqr(weight[i])*scale;
	}
...
void gather_weight_sqr(ArrayNd<OTYPE, n_dist2> &den, particle_dist &pic,
		   PTYPEAVec &weight,  FTYPE *xmin, FTYPE *xmax, 
		   int *nmesh, int *iwrap, FTYPE scale)
{
  ...
  for (int i = 0; i < np; ++i) {
	...
	for (int i1= 0; i1<2; i1++)
	  if (ix[1][i1] >= 0 && ix[1][i1] < nmesh[1])
	    {
	      w=wx[0][i0]*wx[1][i1];
	      den(ix[0][i0],ix[1][i1]) += w*Sqr(weight[i])*scale;
	    }
...
void gather_weight_sqr(ArrayNd<OTYPE, n_dist3> &den, particle_dist &pic, 
		   PTYPEAVec &weight,  FTYPE *xmin, FTYPE *xmax, 
		   int *nmesh, int *iwrap, FTYPE scale)
{
	...
	for (int i1= 0; i1<2; i1++)
	  if (ix[1][i1] >= 0 && ix[1][i1] < nmesh[1])
	      ...
	      if (ix[2][i2] >= 0 && ix[2][i2] < nmesh[2])
		{
		  ...
		  den(ix[0][i0],ix[1][i1],ix[2][i2]) += w*Sqr(weight[i])*scale;
		}
...
void gather_weight_sqr(ArrayNd<OTYPE, n_dist4> &den, particle_dist &pic,
		   PTYPEAVec &weight,  FTYPE *xmin, FTYPE *xmax, 
		   int *nmesh, int *iwrap, FTYPE scale)
{
...
void gather_weight_sqr(ArrayNd<OTYPE, n_dist6> &den, particle_dist &pic, 
		   PTYPEAVec &weight,  FTYPE *xmin, FTYPE *xmax, 
		   int *nmesh, int *iwrap, FTYPE scale)
{
	  ...
	  if (ix[3][i3] >= 0 && ix[3][i3] < nmesh[3])
	      ...
	      if (ix[5][i5] >= 0 && ix[5][i5] < nmesh[5])
		{
		  w=wx[0][i0]*wx[1][i1]*wx[2][i2]*wx[3][i3]*
		    wx[4][i4]*wx[5][i5];
		  den(ix[0][i0], ix[1][i1], ix[2][i2], ix[3][i3],
		      ix[4][i4], ix[5][i5]) += w*Sqr(weight[i])*scale;
		}
