// Multiple gather-domain-scale routines below for 1-D, 2-D 4-D and 6-D 
// The major difference between these routines and the traditional 
// gather-scale is that the wrap routine wraps left edge points to an 
// arbitrary point (iwrapTo), rather than 0. 
...
void gather_domain_scale(ArrayNd<OTYPE, n_dist1> &den, PTYPEAVec *data[], FTYPE *xmin, 
		  FTYPE *xmax, int *nmesh, int *iwrap, int *iwrapTo, FTYPE scale, int np, PTYPEAVec xmissing)
{
  ...
  //  For each particle ... 
  for (int i = 0; i < np; ++i) {
    if (xmissing(i) >= 0) {// This excludes absent particles which should always be <0
    ...
    for (int id = 0; id<n_dist1; ++id) {
      ...
      // for that point)
      if (ix[id][1] == iwrap[id]) ix[id][1] = iwrapTo[id];
...
void gather_domain_scale(ArrayNd<OTYPE, n_dist2> &den, PTYPEAVec* data[], 
		  FTYPE *xmin, FTYPE *xmax, int *nmesh, int *iwrap, int *iwrapTo, 
...
void gather_domain_scale(ArrayNd<OTYPE, n_dist3> &den, PTYPEAVec *data[], FTYPE *xmin, 
		  FTYPE *xmax, int *nmesh, int *iwrap, int *iwrapTo, FTYPE scale, int np, PTYPEAVec xmissing)
{
	...
	for (int i1= 0; i1<2; i1++)
	  if (ix[1][i1] >= 0 && ix[1][i1] < nmesh[1])
	      ...
	      if (ix[2][i2] >= 0 && ix[2][i2] < nmesh[2])
		{
		  ...
		  den(ix[0][i0],ix[1][i1],ix[2][i2]) += w*scale;
		}
...
void gather_domain_scale(ArrayNd<OTYPE, n_dist4> &den, PTYPEAVec *data[], FTYPE *xmin, 
		  FTYPE *xmax, int *nmesh, int *iwrap, int *iwrapTo, FTYPE scale, int np, PTYPEAVec xmissing)
{
	...
	for (int i1= 0; i1<2; i1++)
	  if (ix[1][i1] >= 0 && ix[1][i1] < nmesh[1])
		  ...
		  if (ix[3][i3] >= 0 && ix[3][i3] < nmesh[3])
		{
		  ...
		  den(ix[0][i0],ix[1][i1],ix[2][i2],ix[3][i3]) += w*scale;
		}
...
void gather_domain_scale(ArrayNd<OTYPE, n_dist6> &den, PTYPEAVec *data[], FTYPE *xmin, 
		  FTYPE *xmax, int *nmesh, int *iwrap, int *iwrapTo, FTYPE scale, int np, PTYPEAVec xmissing)
{
