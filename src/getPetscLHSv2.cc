...
PetscErrorCode getPetscLHSv2(Mat &A, FArrayND_ranged &den)
{
  ...
  PetscScalar value[stencil_size];
  PetscInt index[stencil_size];
  ...
  FTYPE c0,c1;
  FTYPE xp1_t0,xp1_t1,xm1_t0,xm1_t1,yp1_t0,yp1_t1,ym1_t0,ym1_t1;
  FTYPE ppz_t0,pmz_t0,mpz_t0,mmz_t0;
  ...
  for (ir=rowStart;ir<rowEnd;ir++) {
    ...
    c0 = dx_sqrInv - dy_sqrInv;
    c1 = 2.0*kappa*dxInv*dyInv;
    xp1_t0 = 0.50*(den_xyz + den_xp1)+
      0.25*(den_ppz + den_yp1 + den_pmz + den_ym1);
    xp1_t1 = 0.25*(den_ppz + den_yp1 - den_pmz - den_ym1);
    yp1_t0 = 0.50*(den_xyz + den_yp1)+
      0.25*(den_mpz + den_xm1 + den_ppz + den_xp1);
    yp1_t1 = 0.25*(den_mpz + den_xm1 - den_ppz - den_xp1);
    xm1_t0 = 0.50*(den_xyz + den_xm1)+
      0.25*(den_ym1 + den_mmz + den_yp1 + den_mpz);
    xm1_t1 = 0.25*(den_ym1 + den_mmz - den_yp1 - den_mpz);
    ym1_t0 = 0.50*(den_xyz + den_ym1)+
      0.25*(den_xp1 + den_pmz + den_xm1 + den_mmz);
    ym1_t1 = 0.25*(den_xp1 + den_pmz - den_xm1 - den_mmz);
    ppz_t0 = 0.25*(den_ppz + den_xp1 + den_yp1 + den_xyz);
    pmz_t0 = 0.25*(den_xp1 + den_pmz + den_xyz + den_ym1);
    mpz_t0 = 0.25*(den_yp1 + den_xyz + den_mpz + den_ym1);
    mmz_t0 = 0.25*(den_xyz + den_ym1 + den_xm1 + den_mmz);
    ...
    if (testLHS) {
      value[count] = 2.0*(HyHzdHx + HxHzdHy);
      if (NDIM == 3) value[count] += 2.0*HxHydHz;
    }
    count++;

    // Forward 1 in X
    index[count] = iyg + ixgp1*ny + izg*ny*nx*nsubdomains;
    value[count] = value_diagInv*0.25*(c0*xp1_t0 + c1*xp1_t1);
    if (testLHS) {
      value[count] = -HyHzdHx;
    }
    count++;
    ...
    value[count] = value_diagInv*0.25*(c0*xm1_t0 + c1*xm1_t1);
    if (testLHS) {
      value[count] = -HyHzdHx;
    }
    count++;
    ...
    value[count] = value_diagInv*0.25*(-c0*yp1_t0 + c1*yp1_t1);
    if (testLHS) {
      value[count] = -HxHzdHy;
    }
    count++;
    ...
    value[count] = value_diagInv*0.25*(-c0*ym1_t0 + c1*ym1_t1);
    if (testLHS) {
      value[count] = -HxHzdHy;
    }
    count++;
    ...
    if (ndim == 3) {
      ...
      value[count] = value_diagInv*0.50*dz_sqrInv*(1+kappa*kappa)*(den_xyz + den_zp1);
      if (testLHS) {
      	value[count] = -HxHydHz;
      }      
      count++;
      ...
      value[count] = value_diagInv*0.50*dz_sqrInv*(1+kappa*kappa)*(den_xyz + den_zm1);
      if (testLHS) {
      	value[count] = -HxHydHz;
      }
      count++;
    ...
    value[count] = value_diagInv*0.25*(dx_sqrInv + dy_sqrInv)*ppz_t0;
    if (testLHS) {
      value[count] = 0.0;
    }
    count++;
    ...
    value[count] = value_diagInv*0.25*(dx_sqrInv + dy_sqrInv)*pmz_t0;
    if (testLHS) {
      value[count] = 0.0;
    }
    count++;
    ...
    value[count] = value_diagInv*0.25*(dx_sqrInv + dy_sqrInv)*mpz_t0;
    if (testLHS) {
      value[count] = 0.0;
    }
    count++;
    ...
    value[count] = value_diagInv*0.25*(dx_sqrInv + dy_sqrInv)*mmz_t0;
    if (testLHS) {
      value[count] = 0.0;
    }
    count++;
    ...
    perr=MatSetValues(A,1,&ir,count,index,value,INSERT_VALUES);CHKERRQ(perr);
