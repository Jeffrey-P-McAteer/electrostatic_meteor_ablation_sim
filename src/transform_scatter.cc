		       ...
		       FTYPE &vrelmax, int &iran, FTYPE &dW, particle_dist &dist_out) 
{
  ...
  FTYPE nptry = 1.0*(dist.np-dist.nabsent)*coll_frac * vrelmax / 
    ((vth_n+v0_n)*sqrt(m_neutral/dist.m));

  if (nptry > (dist.np-dist.nabsent)) {
    char message[128];
    sprintf(message,"%s ( %g > %Ld : %d )\n",
	    "transform_scatter attempting to collide more particles than present",
	    nptry, dist.np-dist.nabsent, dist.nabsent);
    terminate(-1,message);
  }
  ...
  // We need a list of particles that will be placced in particle id1
  int ndim_place=NDIM+3; // Only works for 3-D velocities
  int i_nptry=nptry;
  if (ran3(&iran) < (nptry-i_nptry) ) i_nptry += 1;

  if (i_nptry > 0) {

    PTYPEAVec place(i_nptry*ndim_place);
    int nplace=0;

    //  For each of these particles...
    for (int iptry = 0; iptry < i_nptry; iptry++) {
      ...
      if (Sqr(ran3(&iran)) < vrelsqr/vrelmaxsqr) {
